...
public class FullFeatureNucleotideDinucleotideShapeNoFlank implements DynamicProgramming {
	...
	public FullFeatureNucleotideDinucleotideShapeNoFlank(int l, int k, boolean isNSBinding,
		...
		int fsLFlankInit = (int) (leftFlank&subFeatureMask);
		int rsLFlankInit = (int) (leftFlankRC&subFeatureMask);
	...
	public double recursiveZ() {
		...
		for (int index=l; index<l+Math.max(r0k-1,2); index++) {
			shapeStartOffset = (index-k-1 < 0) ? 0 : index-k-1;
			shapeEndOffset 	 = (index-2 > maxFrames-1) ? maxFrames-1 : index-2;
			for (int i=0; i<maxFrames; i++) {
				for (int j=0; j<maxSubFeatures; j++) {
			...
			for (long currSubFeat=0; currSubFeat<maxSubFeatures; currSubFeat++) {
				...
				for (int currOffset=0; currOffset<maxFrames; currOffset++) {
					position = index-currOffset;
					shapePosition = position-2;
					if (index < l+2 && currOffset>=shapeStartOffset && currOffset <= shapeEndOffset) {
						fShapeWeight = 0;
						rShapeWeight = 0;
						fShapeIndex = (int) (fNewWorkFeat & 1023);
						rShapeIndex = (int) (rNewWorkFeat & 1023);
					...
					fsWork[currOffset][(int) (fNewWorkFeat & subFeatureMask)] +=
							fsOffsets[currOffset][(int) currSubFeat]*fR0Weight*fShapeWeight;
					rsWork[currOffset][(int) (rNewWorkFeat & subFeatureMask)] +=
							rsOffsets[currOffset][(int) currSubFeat]*rR0Weight*rShapeWeight;
				}
			}
			for (int i=0; i<maxFrames; i++) {
				for (int j=0; j<maxSubFeatures; j++) {
	...
	public void recursiveHessian() {
		...
		for (int index=0; index<l; index++) {								//R0, NUC
			...
			shapeStartOffset	= (index-k-1 < 0) ? 0 : index-k-1;
			shapeEndOffset		= (index-2 > l-k) ? l-k : index-2;
			for (int i=0; i<maxFrames; i++) {
				for (int j=0; j<maxSubFeatures; j++) {
					fsWork[i][j] = 0;
					rsWork[i][j] = 0;
		...
		for (int index=l; index<l+Math.max(r0k-1,2); index++) {
			shapeStartOffset	= (index-k-1 < 0) ? 0 : index-k-1;
			shapeEndOffset		= (index-2 > l+k-2) ? l+k-2 : index-2;
			for (int i=0; i<maxFrames; i++) {
				for (int j=0; j<maxSubFeatures; j++) {
					fsWork[i][j] = 0;
					rsWork[i][j] = 0;
